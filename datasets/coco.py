...
class DatasetCOCO(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize, seed, num_test_samples=0):
        ...
        with open('datasets/splits/coco/id_map.txt','r') as f:
            cats = f.readlines()
            cats = [i.replace('\n','') for i in cats]
            self.cats = [i.split(':')[-1] for i in cats]
    ...
    def __getitem__(self, idx):
        ...
        category = self.cats[class_sample]
    ...
    def read_img(self,name):
        return(Image.open(os.path.join(self.base_path, name)).convert('RGB'))
    ...
    def load_frame(self):
        ...
        while True:
            ...
            if query_mask.shape[0] > 0:  # at least two instance exists
                break
