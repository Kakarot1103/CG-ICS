...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize, seed, num_test_samples=0):
        ...
        self.cats = [['plane', 'bike', 'bird', 'boat', 'bottle',
    ...
    def __len__(self):
        if self.split == 'trn':
            ...
            # 0 means use default value (1000)
    ...
    def __getitem__(self, idx):
        ...
        batch = {'query_img': query_img, #3,h,w
                 ...
                 'query_ignore_idx': query_ignore_idx, #h,w

                 'org_query_imsize': org_qry_imsize, #h,w

                 'support_imgs': support_imgs, # 1,3,h,w
                 'support_masks': support_masks, # 1,h,w
                 'support_names': support_names,  #[name]
                 'support_img_paths': [os.path.join(self.img_path, name) + '.jpg' for name in support_names],
                 'support_ignore_idxs': support_ignore_idxs, #[1,1024,1024]
                 'category': self.cats[class_sample % 5],
    ...
    def load_frame(self, query_name, support_names):
        ...
        query_mask = self.read_mask(query_name,1)
        support_imgs = [self.read_img(name) for name in support_names]
        support_masks = [self.read_mask(name,1) for name in support_names]
