...
class DatasetPASCALPart(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize, seed=0, box_crop=True, num_test_samples=0):
        self.split = 'val' if split in ['val', 'test'] else 'train'
        self.cat = ['animals', 'person', 'indoor','vehicles'][fold]
        ...
        for i, obj in enumerate(self.cat_annos['object']):
            for part in self.cat_annos['object'][obj]['part']:
                if len(self.cat_annos['object'][obj]['part'][part]['train']) >= shot + 20 and \
                        len(self.cat_annos['object'][obj]['part'][part]['val']) >= shot + 20:
    ...
    def __len__(self):
        if self.split == 'trn':
            ...
            # 0 means use default value (min(len(img_metadata), 1000))
            if self.num_test_samples != 0:
                return self.num_test_samples
            else:
                return min(len(self.img_metadata), 1000)
    ...
    def __getitem__(self, idx):
        ...
        class_sample, org_qry_imsize,neg_query_mask  = self.sample_episode(idx)
        ...
        neg_query_mask = torch.from_numpy(neg_query_mask).float()
        if not self.use_original_imgsize:
            neg_query_mask = F.interpolate(neg_query_mask.unsqueeze(0).unsqueeze(0).float(), query_img.size()[-2:], mode='nearest').squeeze()
        ...
        batch = {'query_img': query_img,
                 ...
                 'neg_query_mask': neg_query_mask,
                 'query_img_path': self.img_file.format(query_img_id),
                 'query_name': query_img_id,
                 'org_query_imsize': org_qry_imsize,
                 'support_imgs': support_imgs,
                 'support_masks': support_masks,
                 'support_img_paths': [self.img_file.format(sup_img_id) for sup_img_id in support_img_ids],
                 'support_names': support_img_ids,
                 'category': class_sample.replace('+',' ').lower(),
    ...
    def read_mask(self, img_name,class_sample):
        anno = json.load(open(self.anno_file.format(img_name), 'r'))
        class_sample = self.cat_part_name[class_sample]
        ...
        part_mask = part_mask[query_obj_box[1]:query_obj_box[3], query_obj_box[0]:query_obj_box[2]]

        # convert to tensor
        part_mask = torch.from_numpy(part_mask).float()
        
        return part_mask
    
    def read_img(self, img_name):

        return Image.open(self.img_file.format(img_name)).convert('RGB')
    ...
    def sample_episode(self, idx):
        ...
        while True:
            ...
            # Negative mask is the complement of the part mask
            neg_part_mask = ~part_mask
        ...
        while True:  # keep sampling support set if query == support
            ...
            while True:
                support_img_id = np.random.choice(self.cat_annos['object'][obj_n]['part'][part_n][self.split], 1, replace=False)[0]
                if len(self.cat_annos['object'][obj_n]['part'][part_n][self.split]) > 1:
        ...
        if self.box_crop:
            ...
            neg_query_mask = neg_query_mask[query_obj_box[1]:query_obj_box[3], query_obj_box[0]:query_obj_box[2]]
        ...
        return query_img, query_mask, support_imgs, support_masks, query_img_id, support_img_ids, class_sample, org_qry_imsize,neg_query_mask
